...
namespace SimpleDeFence.Utilities
{
    public sealed class IpAddrMask : IEquatable<IpAddrMask>
    {
        ...
        /// <summary>True when traffic to or from this address never leaves the local network:
        /// loopback, the RFC1918 private ranges, link-local, IPv6 unique-local (fc00::/7) and
        /// multicast. Used by the Connections page to mark local traffic apart from traffic
        /// heading out to the internet.
        ///
        /// The unspecified address is deliberately NOT local. A socket listening on 0.0.0.0 or ::
        /// is bound to every interface, which is the opposite of confined to this machine, and
        /// calling that "local" in the UI would say the reassuring thing about the exposed case.</summary>
        public static bool IsLocalNetwork(IPAddress? address)
        {
            ...
            var mask = new IpAddrMask(address);
            if (mask.IsLinkLocal || mask.IsMulticast)
                return true;
            ...
            // fc00::/7 - unique local addresses, the IPv6 counterpart of the ranges above.
            return (bytes[0] & 0xFE) == 0xFC;
        }

        /// <summary>Convenience overload for the string form the connection lists carry. A value
        /// that will not parse is not local: an address we cannot read is not one we can vouch
        /// for.</summary>
        public static bool IsLocalNetwork(string? address)
            => !string.IsNullOrWhiteSpace(address)
               && IPAddress.TryParse(address, out var parsed)
               && IsLocalNetwork(parsed);
