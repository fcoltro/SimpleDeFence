...
namespace SimpleDeFence
{
    ...
    public static class FirewallModes
    {
        ...
        /// <summary>True for the five modes the firewall can actually operate in. False for anything
        /// else, including FirewallMode.Unknown - which is a client-side "no state yet" sentinel and
        /// the initial value of ServerState.Mode, never a mode the service should run in. The service
        /// checks this before accepting a mode over the pipe, because a mode it does not recognise
        /// assembles no default rules at all.</summary>
        public static bool IsOperatingMode(FirewallMode mode) => IndexOf(mode) >= 0;
