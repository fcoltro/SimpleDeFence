...
<!--
  ...
  2. Harvest (from the MsiSetup directory; adjust the WiX Toolset path to the local install):
       ...
       heat.exe dir "..\SimpleDeFence\bin\Release\net10.0-windows10.0.19041.0\win-x64\publish" -platform x64 -cg PublishedDependencies -ag -scom -sreg -srd -dr INSTALLDIR -var var.PublishDir -t HarvestPublishDir.xslt -out Dependencies.wxs

     -cg PublishedDependencies  names the generated ComponentGroup; Product.wxs's <Feature>
                                references exactly this Id.
     -ag                        author Guid="*" and let the linker derive each component GUID from
                                its key path, so the same file at the same location keeps the same
                                identity from one build to the next.

                                This was -gg, which writes a freshly minted GUID for every component
                                on every harvest. The note here used to claim that was safe because
                                RemoveExistingProducts runs After='InstallValidate'. It is not, and
                                the failure is ugly: upgrading between two independently harvested
                                builds hangs in the commit phase with SimpleDeFence.exe /install
                                stuck in session 0, and killing the installer rolls back into a
                                half-removed install - no service, no WFP filters, ~400 of 587 files.
                                Reproduced on the test VM going from a CI-built package to a locally
                                built one. Two packages sharing one harvest upgrade cleanly, which is
                                what pins it on the GUID churn rather than on the sequencing.
